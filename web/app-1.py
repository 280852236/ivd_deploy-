...
def store_analysis_result(analysis_id, data):
    for attempt in range(3):
        try:
            ...
            r.set(key, json.dumps(data, ensure_ascii=False), ex=Config.ANALYSIS_TTL_HOURS * 3600)
            return
        except Exception as e:
...
def analyze_text(text: str, rules: List[Dict], skip_motor_status: bool = False) -> List[Dict]:
    if not text:
        return []
    matched = []
...
def process_text_file_from_bytes(file_bytes: bytes, filename: str, rules: List[Dict]) -> Dict:
    ...
    return _process_text_content(content, filename, rules, MAX_FILE_CONTENT, MAX_LINES)
...
def _process_text_content(content: str, filename: str, rules: List[Dict], MAX_FILE_CONTENT: int, MAX_LINES: int) -> Dict:
    ...
    if is_receive_file or file_type == 'unknown':
        ...
        analysis = analyze_text(original_content, rules, skip_motor_status=False)
    elif is_aspiration_file:
        analysis = analyze_text(original_content, rules, skip_motor_status=True)
    else:
        analysis = analyze_text(original_content, rules, skip_motor_status=False)
...
def process_zip_file(file, rules: List[Dict], batch_size: int = 0, start_index: int = 0) -> Dict:
    MAX_FILES = 2000
        ...
        for raw_name in relevant_raw:
            ...
            try:
                ...
                if is_receive_file:
                    ...
                    file_analysis = analyze_text(content_for_analysis, rules, skip_motor_status=False)
                elif is_aspiration_file:
                    file_analysis = analyze_text(content_for_analysis, rules, skip_motor_status=True)
                else:
                    file_analysis = analyze_text(content_for_analysis, rules, skip_motor_status=False)
...
def load_more_files(analysis_id):
    ...
    try:
        ...
        with open(temp_path, 'rb') as f:
            batch_result = process_zip_file(f, rules, batch_size=500, start_index=next_index)
...
def get_analysis_file(analysis_id):
    ...
    if not data:
        return jsonify({'error': '分析结果不存在或已过期'}), 404
    file_data = data['files'].get(filename)
    if not file_data:
        decoded = unquote(filename)
        file_data = data['files'].get(decoded)
    if not file_data:
